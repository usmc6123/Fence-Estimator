      ...
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-xs text-[#666666]">
            Enter your email and password credentials to access the estimators and estimators dossiers.
          </p>
        </div>
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-2xl p-6 shadow-xl">
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-american-blue hover:bg-american-blue/90 text-white py-3 text-xs font-black uppercase tracking-widest shadow-md transition-all disabled:opacity-50 hover:cursor-pointer"
            >
...
// 4. Combined Unified Pricing Plans & Billing Component
interface PricingPlansAndBillingProps {
  userId: string | null;
  userEmail: string | null;
  currentTier: 'free' | 'paid';
  nextBillingDate: string | null;
...
export function PricingPlansAndBilling({ userId, userEmail, currentTier, nextBillingDate, onGetStarted }: PricingPlansAndBillingProps) {
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState('');
  const [msg, setMsg] = React.useState('');
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-6">
      ...
      <div className="mb-8 p-6 bg-white border border-[#E5E5E5] rounded-2xl shadow-sm">
        <h1 className="text-2xl font-black uppercase tracking-tight text-american-blue">Pricing Plans & Billing</h1>
        <p className="text-xs text-[#666666] mt-1">
          Manage your subscription level, verify billing dates, and view plan specifications in one consolidated engine terminal.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3 mb-10">
        ...
        <div className="md:col-span-2 rounded-2xl border border-[#E5E5E5] bg-white p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm text-[#1A1A1A] leading-relaxed">
            {currentTier === 'paid' 
              ? 'You have complete access to Lone Star Fence Works Premium features, including automated contracts, comprehensive PDF takeaways, and unlimited dossier slots.'
              : 'You are currently on the free plan limiting saved records and custom takeoff calculations. Review the premium tier specifications options below to expand capacity.'
            }
          </p>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between">
            ...
            {currentTier === 'free' ? (
              <span className="text-xs font-medium text-[#666666] select-none italic">
                Choose the Premium Paid Tier below to expand capacity
              </span>
            ) : (
              <button
                onClick={handleCancel}
      ...
      {/* Grid: Plan Comparison Options */}
      <div className="mb-6">
        <h2 className="text-xl font-black uppercase tracking-tight text-american-blue mb-2">Plan Details & Comparison</h2>
        <p className="text-xs text-[#666666] mb-6">Compare specifications of active tiers and switch options instantly.</p>
        
        <div className="grid gap-8 md:grid-cols-2">
          {/* Free Plan */}
          <div className="flex flex-col justify-between rounded-2xl border border-[#E5E5E5] bg-white p-8 shadow-sm relative overflow-hidden transition-all">
            <div>
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-lg font-black uppercase text-[#666666]">Free Tier</h3>
          ...
          {/* Paid Plan */}
          <div className="flex flex-col justify-between rounded-2xl border-2 border-american-blue bg-white p-8 shadow bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-american-blue/5 via-white to-white relative overflow-hidden transition-all">
            <div className="absolute top-0 right-0 bg-american-blue text-white text-[9px] font-black uppercase tracking-widest px-4 py-1.5 rounded-bl-xl flex items-center gap-1">
              <Sparkles size={10} />
              Recommended
            </div>
            ...
            <div className="mt-8">
              {currentTier === 'paid' ? (
                <button
                  disabled
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-green-500 text-white py-3.5 text-xs font-black uppercase tracking-widest cursor-default font-bold"
                >
                  Active Subscription
                </button>
              ) : (
                <button
                  onClick={handleSubscribe}
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-american-blue py-3.5 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-american-blue/20 hover:bg-american-blue/90 disabled:opacity-50 transition-all font-bold"
                >
                  {loading ? <Loader2 className="animate-spin" size={14} /> : <CreditCard size={14} />}
                  Subscribe for $50/mo
                </button>
