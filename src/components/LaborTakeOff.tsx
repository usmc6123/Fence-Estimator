...
export default function LaborTakeOff({ 
  ...
  const handleResyncGhlCalendar = async () => {
    ...
    console.log(`[FRONTEND ACTION TRACE] resync-ghl-calendar click handler
      scheduleSyncTraceId: ${scheduleSyncTraceId}
      estimateId: ${estimate.id}
      customerName: ${estimate.customerName || 'N/A'}
      selected start date: ${estimate.scheduledStartDate || 'N/A'}
      selected duration/install days: ${estimate.scheduledDuration || (estimate as any).installDuration || '1 day'}
      selected crew: ${estimate.assignedCrew || 'N/A'}
      action being called: resync-ghl-calendar
    ...
    try {
      ...
      if (onUpdateEstimate) {
        onUpdateEstimate({
          ...
          ghlCalendarSyncError: resData.ghlCalendarSyncError,
          ghlCalendarLastSyncedAt: new Date().toISOString(),
          ghlSyncDebug: resData.ghlSyncDebug || null
        } as any);
      }
    } catch (err: any) {
      setScheduleError(err.message || String(err));
          ...
          {isEditingSchedule ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs font-bold text-slate-700">
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                {/* GHL Sync Trace Panel */}
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <div className="flex items-center gap-2 mb-2">
                    <Terminal size={12} className="text-slate-500" />
                    <span className="text-[10px] font-black uppercase text-slate-700 tracking-wider">GHL Sync Trace (Admin-Only Debug)</span>
                  </div>

                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 space-y-2 overflow-hidden text-[10px] font-mono text-slate-300">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <span className="text-slate-500 block text-[8px] uppercase font-bold">Sync Status</span>
                        <span className={cn(
                          "font-bold",
                          (estimate as any).ghlSyncDebug?.status === 'success' 
                            ? "text-emerald-400" 
                            : (estimate as any).ghlSyncDebug?.status === 'failed' 
                              ? "text-rose-400" 
                              : "text-amber-400"
                        )}>
                          {((estimate as any).ghlSyncDebug?.status || estimate.ghlCalendarSyncStatus || 'UNKNOWN').toUpperCase()}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 block text-[8px] uppercase font-bold">Trace ID</span>
                        <span className="text-slate-300 break-all">{(estimate as any).ghlSyncDebug?.scheduleSyncTraceId || 'N/A'}</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2 border-t border-slate-800/60 pt-1.5">
                      <div>
                        <span className="text-slate-500 block text-[8px] uppercase font-bold">Last Step Completed</span>
                        <span className="text-slate-300 text-[9px] font-sans">
                          {(() => {
                            const steps = (estimate as any).ghlSyncDebug?.steps || [];
                            const lastSuccess = [...steps].reverse().find((s: any) => s.status === 'success');
                            return lastSuccess ? lastSuccess.step : 'None';
                          })()}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 block text-[8px] uppercase font-bold">Failed Step</span>
                        <span className="text-rose-400 text-[9px] font-sans font-medium">
                          {(() => {
                            const steps = (estimate as any).ghlSyncDebug?.steps || [];
                            const failedStep = steps.find((s: any) => s.status === 'failed');
                            return failedStep ? failedStep.step : 'None';
                          })()}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-800/60 pt-1.5">
                      <span className="text-slate-500 block text-[8px] uppercase font-bold">Error Message</span>
                      <p className="text-rose-400 text-[9px] font-sans leading-tight mt-0.5 whitespace-pre-wrap font-medium">
                        {(estimate as any).ghlSyncDebug?.errors?.[0] || 
                         ((estimate as any).ghlSyncDebug?.steps || []).find((s: any) => s.status === 'failed')?.reason || 
                         estimate.ghlCalendarSyncError ||
                         'None'}
                      </p>
                    </div>

                    <div className="border-t border-slate-800/60 pt-1.5">
                      <span className="text-slate-500 block text-[8px] uppercase font-bold">Appointment IDs Created</span>
                      <span className="text-emerald-400 font-bold tracking-tight text-[9px] break-all">
                        {(() => {
                          const ids = (estimate as any).ghlCalendarEventIds || (estimate.ghlCalendarEventId ? [estimate.ghlCalendarEventId] : []);
                    ...
                    <div className="border-t border-slate-800/60 pt-1.5">
                      <span className="text-slate-500 block text-[8px] uppercase font-bold">Raw GHL Sync Flow (Steps)</span>
                      <div className="mt-1 space-y-1 max-h-32 overflow-y-auto pr-1">
                        {(((estimate as any).ghlSyncDebug?.steps) || []).map((step: any, idx: number) => (
                          <div key={idx} className="flex items-start justify-between text-[8px] font-mono leading-none py-0.5 border-b border-slate-800/30">
                    ...
                    <div className="border-t border-slate-800/60 pt-2 flex items-center justify-between">
                      <span className="text-[7px] text-slate-500 font-sans">
                        Last Action: {(estimate as any).ghlSyncDebug?.actionName || 'None'}
                      </span>
                      <button
                        type="button"
                        onClick={handleResyncGhlCalendar}
                        disabled={scheduleSubmitting}
                        className="bg-slate-850 hover:bg-slate-800 text-white font-sans font-bold text-[9px] px-2.5 py-1 rounded border border-slate-700 hover:border-slate-600 transition-all flex items-center gap-1 active:scale-95 disabled:opacity-50"
                      >
                        <RefreshCw size={8} className={cn(scheduleSubmitting && "animate-spin")} />
                        Retry GHL Sync
                      </button>
                    </div>
                  </div>
