...
if (admin.apps.length === 0) {
  ...
  if (firebaseConfigEnv) {
    try {
      ...
      console.error('Error parsing FIREBASE_CONFIG env in test email handler:', error);
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    console.error('Test outer failure:', error);
    return res.status(500).json({ error: error.message || 'Outer API error.' });
  }
}
