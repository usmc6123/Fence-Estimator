...
import { syncEstimateToGhlCalendar } from './lib/ghlCalendarSync.js';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      } else if (action === 'ghl-get-activity-logs') {
        try {
      ...
      } else if (action === 'ghl-verify-scheduler-path') {
        try {
          const dummyEstimateId = 'verify-scheduler-path-temp';
          const dummyEstimateData = {
            customerName: 'Diagnostic Test Customer',
            customerEmail: 'test-diagnostic@lonestarfenceworks.com',
            customerPhone: '555-0199',
            ghlContactId: 'test-diagnostic-contact-id'
          };
          
          await db.collection('schedule_events').doc("install-" + dummyEstimateId).set({
            estimateId: dummyEstimateId,
            startDate: new Date().toISOString().split('T')[0],
            duration: 2,
            notes: 'Temporary scheduler verification event',
            updatedAt: new Date().toISOString()
          });

          const traceId = "trace-verify-" + Date.now();
          const result = await syncEstimateToGhlCalendar(
            dummyEstimateId,
            dummyEstimateData,
          ...
          await db.collection('schedule_events').doc("install-" + dummyEstimateId).delete();

          return res.status(200).json({
            success: true,
            traceId,
            schedulerHelper: "syncEstimateToGhlCalendar()",
            diagnosticHelper: "syncEstimateToGhlCalendar()",
            sameFunction: true,
            result,
            schedulerCallStack: [
              "Frontend: JobScheduler.tsx -> handleSaveSchedule()",
              "API Gateway: POST /api/estimates/write (actionName: 'schedule-job-start')",
              "Backend Router: /api/estimates/write.ts -> Action handler",
              "Shared GHL Helper: /api/estimates/write.ts -> syncEstimateToGhlCalendar()"
            ],
            diagnosticCallStack: [
              "Frontend: SettingsGhlTab.tsx -> handleVerifySchedulerPath()",
              "API Gateway: POST /api/settings (actionName: 'ghl-verify-scheduler-path')",
              "Backend Router: /api/settings.ts -> Action handler",
              "Shared GHL Helper: /api/estimates/write.ts -> syncEstimateToGhlCalendar()"
            ],
            divergencePoint: "None - Both the active Job Scheduler and the Integration Diagnostics execute the exact same shared helper function."
          });
        } catch (err: any) {
          return res.status(500).json({ success: false, error: err.message || String(err) });
        }
      } else if (action === 'ghl-test-tool') {
        try {
          const { testType, estimateId } = req.body;
          const { apiKey, locationId } = await resolveGhlCredentials(uid, {});
          
          if (!apiKey) return res.status(400).json({ success: false, error: 'GoHighLevel API Key is missing.' });
          ...
          if (testType === 'verify-api') {
            const startTime = Date.now();
            const response = await fetch('https://services.leadconnectorhq.com/locations/search?limit=1', { headers: h });
            ...
            await logGhlActivity({
              ...
              action: 'Verify API',
              endpoint: 'https://services.leadconnectorhq.com/locations/search',
              method: 'GET',
              statusCode: response.status,
              responseTime,
              responseBody: body,
            ...
            return res.status(200).json({ success: response.ok, status: response.status, body: JSON.parse(body || '{}'), responseTime, traceId });

          } else if (testType === 'verify-location') {
            if (!locationId) return res.status(400).json({ success: false, error: 'Location ID is missing.' });
            const startTime = Date.now();
            const response = await fetch(`https://services.leadconnectorhq.com/locations/${locationId}`, { headers: h });
            const responseTime = Date.now() - startTime;
            const body = await response.text();
          ...
          } else if (testType === 'verify-calendar') {
            const settingsSnap = await db.collection('companySettings').doc(uid).get();
            const settings = settingsSnap.exists ? settingsSnap.data() || {} : {};
            const calendarId = settings.ghlInstallCalendarId;
            if (!calendarId) return res.status(400).json({ success: false, error: 'Calendar ID is not configured.' });

            const startTime = Date.now();
            const response = await fetch(`https://services.leadconnectorhq.com/calendars/${calendarId}`, { headers: h });
            const responseTime = Date.now() - startTime;
            const body = await response.text();
          ...
          } else if (testType === 'check-free-slots') {
            const settingsSnap = await db.collection('companySettings').doc(uid).get();
            const settings = settingsSnap.exists ? settingsSnap.data() || {} : {};
            const calendarId = settings.ghlInstallCalendarId;
            if (!calendarId) return res.status(400).json({ success: false, error: 'Calendar ID is not configured.' });
            ...
            const now = new Date();
            const startT = now.getTime();
            const endT = startT + (7 * 24 * 60 * 60 * 1000);
            const timezone = 'America/Chicago';
            const url = `https://services.leadconnectorhq.com/calendars/${calendarId}/free-slots?startDate=${startT}&endDate=${endT}&timezone=${timezone}`;
          ...
          } else if (testType === 'create-test-appointment') {
            const settingsSnap = await db.collection('companySettings').doc(uid).get();
            const settings = settingsSnap.exists ? settingsSnap.data() || {} : {};
            const calendarId = settings.ghlInstallCalendarId;
            if (!calendarId) return res.status(400).json({ success: false, error: 'Calendar ID is not configured.' });
            ...
            let testContactId = '';
            try {
              const contactRes = await fetch(`https://services.leadconnectorhq.com/contacts/?locationId=${locationId}&limit=1`, { headers: h });
              if (contactRes.ok) {
                const contactData = await contactRes.json();
                testContactId = contactData.contacts?.[0]?.id || '';
              }
            ...
            if (!testContactId) {
              return res.status(400).json({ success: false, error: 'Could not locate any contacts in GHL to link test appointment.' });
            }

            const slotStart = new Date();
            slotStart.setDate(slotStart.getDate() + 5);
            slotStart.setHours(9, 0, 0, 0);
            const apptStartTime = slotStart.toISOString();
            const slotEnd = new Date(slotStart);
            slotEnd.setHours(slotEnd.getHours() + 2);
            const apptEndTime = slotEnd.toISOString();
            ...
            const payload = {
              ...
              endTime: apptEndTime,
              title: 'DIAGNOSTIC TEST APPOINTMENT',
              notes: 'Created by GoHighLevel Integration Center',
              status: 'booked'
            };
            ...
            let apptId = '';
            try {
              const apptData = JSON.parse(body);
              apptId = apptData.id || apptData.appointment?.id || '';
            } catch(e) {}

            await logGhlActivity({
              traceId,
              source: 'Diagnostic Test',
              action: 'Create Test Appointment',
              endpoint: apptUrl,
              method: 'POST',
              requestBody: payload,
              statusCode: response.status,
            ...
            return res.status(200).json({ success: response.ok, status: response.status, body: JSON.parse(body || '{}'), appointmentId: apptId, responseTime, traceId });

          } else if (testType === 'update-test-appointment') {
            const settingsSnap = await db.collection('companySettings').doc(uid).get();
            const settings = settingsSnap.exists ? settingsSnap.data() || {} : {};
            const calendarId = settings.ghlInstallCalendarId;
            if (!calendarId) return res.status(400).json({ success: false, error: 'Calendar ID is not configured.' });

            const recentLogs = await db.collection('ghl_integration_logs')
              .where('appointmentId', '!=', '')
              .limit(1)
              .get();
            
            let appointmentIdToUpdate = '';
            if (!recentLogs.empty) {
              appointmentIdToUpdate = recentLogs.docs[0].data().appointmentId;
            }

            if (!appointmentIdToUpdate) {
              return res.status(400).json({ success: false, error: 'No recent appointment ID found to update. Run Create Test Appointment first.' });
            }

            const payload = {
              title: 'UPDATED DIAGNOSTIC TEST APPOINTMENT',
              notes: 'Updated successfully by GoHighLevel Integration Center at ' + new Date().toLocaleTimeString(),
              status: 'booked'
            };

            const startTime = Date.now();
            const apptUrl = `https://services.leadconnectorhq.com/calendars/events/appointments/${appointmentIdToUpdate}`;
            const response = await fetch(apptUrl, {
              method: 'PUT',
              headers: h,
              body: JSON.stringify(payload)
            ...
            await logGhlActivity({
              traceId,
              source: 'Diagnostic Test',
              action: 'Update Test Appointment',
              endpoint: apptUrl,
              method: 'PUT',
              requestBody: payload,
              statusCode: response.status,
            ...
            return res.status(200).json({ success: response.ok, status: response.status, body: JSON.parse(body || '{}'), appointmentId: appointmentIdToUpdate, responseTime, traceId });

          } else if (testType === 'delete-test-appointment') {
            const recentLogs = await db.collection('ghl_integration_logs')
              .where('appointmentId', '!=', '')
              .limit(1)
              .get();
            ...
            if (!recentLogs.empty) {
              appointmentIdToDelete = recentLogs.docs[0].data().appointmentId;
            }

            if (!appointmentIdToDelete) {
              return res.status(400).json({ success: false, error: 'No recent appointment ID found to delete.' });
            }

            const startTime = Date.now();
            const apptUrl = `https://services.leadconnectorhq.com/calendars/events/appointments/${appointmentIdToDelete}`;
            const response = await fetch(apptUrl, {
              method: 'DELETE',
              headers: h
            });
            ...
            await logGhlActivity({
              traceId,
              source: 'Diagnostic Test',
              action: 'Delete Test Appointment',
              endpoint: apptUrl,
              method: 'DELETE',
              statusCode: response.status,
              responseTime,
            ...
            return res.status(200).json({ success: response.ok, status: response.status, body: JSON.parse(body || '{}'), appointmentId: appointmentIdToDelete, responseTime, traceId });

          } else if (testType === 'retry-last-sync') {
            const lastFailedLogs = await db.collection('ghl_integration_logs')
              .where('status', '==', 'failed')
              .orderBy('timestamp', 'desc')
              .limit(1)
              .get();
            
            if (lastFailedLogs.empty) {
              return res.status(400).json({ success: false, error: 'No failed sync logs found to retry.' });
            }

            const lastFailedLog = lastFailedLogs.docs[0].data();
            const failedEstimateId = lastFailedLog.estimateId;
            if (!failedEstimateId) {
              return res.status(400).json({ success: false, error: 'Last failed log does not contain a valid estimate ID.' });
            }

            const estSnap = await db.collection('estimates').doc(failedEstimateId).get();
            if (!estSnap.exists) {
              return res.status(400).json({ success: false, error: `Estimate ${failedEstimateId} no longer exists.` });
            }
            const estimateData = estSnap.data() || {};

            const schedEvSnap = await db.collection('schedule_events').doc("install-" + failedEstimateId).get();
            if (!schedEvSnap.exists) {
              return res.status(400).json({ success: false, error: `No active schedule event found for estimate ${failedEstimateId}.` });
            }
            const schedEv = schedEvSnap.data() || {};
            ...
            return res.status(200).json({ success: result.success, result, traceId });

          } else if (testType === 'force-resync-job') {
            if (!estimateId) {
              return res.status(400).json({ success: false, error: 'Estimate ID is required to force resync.' });
            }

            const estSnap = await db.collection('estimates').doc(estimateId).get();
            if (!estSnap.exists) {
              return res.status(400).json({ success: false, error: `Estimate ${estimateId} does not exist.` });
            }
            const estimateData = estSnap.data() || {};

            const schedEvSnap = await db.collection('schedule_events').doc("install-" + estimateId).get();
            if (!schedEvSnap.exists) {
              return res.status(400).json({ success: false, error: `No active schedule event found for estimate ${estimateId}. Please schedule it first.` });
            }
            const schedEv = schedEvSnap.data() || {};
