...
export async function sendGhlWebhook(
  ...
  try {
    ...
    // 1. Resolve companySettings document
    let settings: any = {};
    ...
    for (const key of lookups) {
      try {
        ...
        console.warn(`Error resolving settings for key ${key}:`, e);
      }
    }

    // 2. Identify the target Webhook URL based on eventType
    let webhookUrl = '';
    ...
    // Fallbacks if event-specific is blank (per requirements, skip if blank, but we can log that we skipped it)
    if (!webhookUrl) {
    ...
    // Log to Firestore in ghlWebhookLog
